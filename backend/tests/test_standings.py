...
def test_provisional_live_standings(client):
    ...
    official_riverside = next(s for s in res_official.json()["standings"] if s["teamId"] == "team-riverside")
...
def test_finish_match_updates_official_standings(client, scorekeeper_headers):
    ...
    riverside_before = next(s for s in res_before.json()["standings"] if s["teamId"] == "team-riverside")
    ...
    riverside_after = next(s for s in res_after.json()["standings"] if s["teamId"] == "team-riverside")

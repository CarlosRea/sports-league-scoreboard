...
async def get_matches(
    ...
    status: str | None = Query(
        None, description="Filter by match status (e.g. IN_PROGRESS, SCHEDULED, FINISHED)"
    ),
    matchday: int | None = Query(None, ge=1, description="Filter by matchday / round number"),
):
...
@router.post(
    "/leagues/{leagueId}/matches", response_model=Match, status_code=status.HTTP_201_CREATED
)
async def create_match(
    leagueId: str, dto: CreateMatchDto, admin_user: User = Depends(require_admin)
):
